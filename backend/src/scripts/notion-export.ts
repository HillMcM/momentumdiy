...
async function main() {
  try {
    ...
    const isPage = /https?:\/\/.+/.test(arg) && !/notion\.so\/.+\?v=/.test(arg); // rough heuristic
    ...
    if (goalTitle) {
      ...
      await NotionSyncService.syncGoalFromPage(goalTitle, arg);
      console.log('Done.');
      process.exit(0);
    }
    ...
    if (isDatabase) {
      ...
      await NotionSyncService.syncMarketing(arg);
      console.log('Done.');
      process.exit(0);
    }
    ...
    if (isPage) {
      ...
      await NotionSyncService.syncContainer(arg);
      console.log('Done.');
      process.exit(0);
    }

    // Fallback: attempt as database ID
