...
/**
 * Seeds or replaces the `content` field for each week of the marketing track
 * titled "Improve social media strategy & engagement" with curated long-form
 * guidance (email-style copy) for Weeks 1–12.
 *
 * This does not alter tasks or titles; it only updates module.content.
 */
async function main() {
  const goalTitle = 'Improve social media strategy & engagement';

  // Map of weekNumber -> content (long form)
  const weekContent: Record<number, string> = {
  ...
  const goal = (goals || []).find(g => (g as any).title?.trim().toLowerCase() === goalTitle.toLowerCase());
  if (!goal) {
    throw new Error(`Goal not found by title: ${goalTitle}`);
  }

  const goalId = (goal as any).id as string;

  // Load modules for this goal to map by week_number
  ...
  // Update content for each week we have text for
  for (const [weekStr, content] of Object.entries(weekContent)) {
    const week = parseInt(weekStr, 10);
    const mod = (modules || []).find(m => (m as any).week_number === week);
    if (!mod) continue; // skip if module missing
